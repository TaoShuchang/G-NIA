...
def main(opts):
    ...
    save_file = 'checkpoint/surrogate_model/' + dataset + '_' + surro_type
    ...
    print("Evaluation GNN Model:", eval_type)
    with torch.autograd.set_detect_anomaly(True):
        # Surrogate model
        if surro_type == 'gcn':
            emb_net = GCN(features.shape[1], 64, labels.max().item() + 1, 0.5).float().to(device)

        elif surro_type == 'gat':
            emb_net = GAT(num_of_layers=2, num_heads_per_layer=[nhead, 1], num_features_per_layer=[nfeat, nhid, nc],
            ...
            emb_net = PPNP(nfeat, nc, [64], 0.5, prop_appnp).to(device)
        emb_net.load_state_dict(torch.load(save_file+'_checkpoint.pt'))

        # Evalution model
        if eval_type == 'gcn':
            eval_net = GCN(features.shape[1], 64, labels.max().item() + 1, 0.5).float().to(device)
        elif eval_type == 'gat':
            eval_net = GAT(num_of_layers=2, num_heads_per_layer=[nhead, 1], num_features_per_layer=[nfeat, nhid, nc],
                add_skip_connection=False, bias=True, dropout=0.6, 
                layer_type=LayerType.IMP2, log_attention_weights=False).to(device)
        else:
            prop_appnp = PPRPowerIteration(alpha=0.1, niter=10)
            eval_net = PPNP(nfeat, nc, [64], 0.5, prop_appnp).to(device)
        eval_net.load_state_dict(torch.load(save_file+'_checkpoint.pt'))
        ...
        if surro_type == 'gcn':
            node_emb = emb_net(feat, nor_adj_tensor)
            W1 = emb_net.gc1.weight.data.detach()
            W2 = emb_net.gc2.weight.data.detach()
        elif surro_type == 'gat':
            adj_topo_tensor = torch.tensor(adj.toarray(), dtype=torch.float, device=device)
            graph_data = (feat, adj_topo_tensor)
            node_emb = emb_net(graph_data)[0]
            W1 = emb_net.gat_net[0].linear_proj.weight.data.detach().t()
            W2 = emb_net.gat_net[1].linear_proj.weight.data.detach().t()
        else:
            node_emb = emb_net(feat, nor_adj_tensor)
            W1 = emb_net.fcs[0].weight.data.detach()
            W2 = emb_net.fcs[1].weight.data.detach().t()
        W = torch.mm(W1, W2).t()    
        
        if eval_type == 'gat':
            graph_data = (feat, adj_topo_tensor)
            logits = eval_net(graph_data)[0]
        else:
            logits = eval_net(feat, nor_adj_tensor)
            ...
            for batch_x,_ in batch_loader:
                ...
                for train_batch in batch_x:
                    ...
                    if eval_type == 'gat':
                        ...
                        new_logits = eval_net((new_feat, new_adj_tensor))[0]
                        ...
                        new_logits = eval_net(new_feat, normalize_tensor(new_adj_tensor))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_type', default='gcn',help='evaluation gnn model')
...
'''
nohup python -u run_gnia.py --suffix multi_gcn --multiedge True --nepochs 10000 --lr 1e-5 --connect True --epsst 50 --epsdec 1 --patience 500 --dataset citeseer --attrtau 1 --edgetau 0.01 --surro_type gcn --eval_type gcn --batchsize 32 > log/white_gcn_gnia/citeseer_multi.log 2>&1 &
'''
